...
if (isset($_POST['submit_profile1'])) {
    ...
    $result = $pdo_statement->execute();
    if ($result) {
        $insert_vitals = "INSERT INTO vitals (info_date, info_time, info_id, patient_name, bp1, pr1, rr1, temp1, heightd, weightd, bmi) 
        VALUES('$datenow', '$timenow', '$profile_id', '$profile_name', '$blood_presure', '$pulse_rate', '$respi', '$temp', '$height', '$weight', '$bmi_all')";
        $insert_vitals_result = $connect->prepare($insert_vitals);
        if ($insert_vitals_result->execute()) {
            $val = "1";

            $query = "UPDATE emp SET track = ?    WHERE id = ?";
            $stmt = $connect->prepare($query);
...
if (isset($_POST['submit_revitals'])) {
    ...
    $User = new User($connect);
    $sql = "INSERT INTO vitals (info_date,info_time,info_id,patient_name,bp1,pr1,rr1,temp1,os1,sugar,heightd,weightd,bmi)
        VALUES ( '$datenow','$timenow','$vital_id1','$vital_name1','$bp1','$pr1','$rr1','$temp1','$os1','$sugar1','$height1','$weight1', '$bmi')";
    $pdo_statement = $connect->prepare($sql);
    $result = $pdo_statement->execute();
